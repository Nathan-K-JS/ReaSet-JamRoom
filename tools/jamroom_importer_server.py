...
def rechord_song(name, chart_url, snap=True, force=False):
    ...
    start = song["start"]
    lines = ["-- generated by the Jam Room importer - do not edit", "return {",
             f"  region = {ji.lua_quote(name)},",
             f"  start = {start:.3f},", f"  [\"end\"] = {song['end']:.3f},",
             "  chords = {"]
    # job["chords"] times are already RELATIVE to the song (Fadr analysed the
    # song's own audio, and the apply script adds the region position when it
    # places them). Subtracting the region start here as well pushed nearly
    # every chord below zero and silently dropped it.
    dropped = 0
    for c in job["chords"]:
        s, e = float(c["start"]), float(c["end"])
        if e > s >= 0:
            lines.append(f"    {{ s = {s:.3f}, e = {e:.3f}, "
    ...
    if not result:
        raise RuntimeError("REAPER did not confirm the chord replacement — "
                           "check its ReaScript console and any open dialog.")
