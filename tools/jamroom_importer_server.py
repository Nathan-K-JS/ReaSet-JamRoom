...
def _push_song_items(cfg, name, song, chords=None, lyric_lines=None):
    """Replace one song's chord and/or lyric items in REAPER.

    Times are written RELATIVE to the region start: Fadr analysed the song's own
    audio, and the REAPER side adds the region position when it places them.
    Subtracting the region start here as well once pushed nearly every chord
    below zero, where it was silently dropped.
    """
    L = ["-- generated by the Jam Room importer - do not edit", "return {",
         f"  region = {ji.lua_quote(name)},",
         f"  start = {song['start']:.3f},",
         f"  [\"end\"] = {song['end']:.3f},"]
    dropped = 0
    if chords is not None:
        ...
        for c in chords:
            ...
            if e > s >= 0:
                L.append(f"    {{ s = {s:.3f}, e = {e:.3f}, "
                         f"name = {ji.lua_quote(c['chord'])} }},")
            else:
                dropped += 1
        L.append("  },")
    if lyric_lines is not None:
        ...
        for ln in lyric_lines:
            ...
            if e > s >= 0 and str(ln.get("text", "")).strip():
                L.append(f"    {{ s = {s:.3f}, e = {e:.3f}, "
                         f"text = {ji.lua_quote(ln['text'])} }},")
        L.append("  },")
    L.append("}")
    if dropped:
        _ui_log(f"NOTE: {dropped} chord entries had no duration and were skipped.")
    (TOOLDIR / "jamroom_pending_rechord.lua").write_text(
        "\n".join(L) + "\n", encoding="utf-8")

    receipt = TOOLDIR / "jamroom_rechord_receipt.txt"
    receipt.unlink(missing_ok=True)
    result = _trigger_reaper_action(cfg, "rechord_cmd", receipt)
    if not result:
        raise RuntimeError("REAPER did not confirm the replacement — check its "
                           "ReaScript console and any open dialog.")
    return result
...
def relyric_song(name, record_id=None, offset=None):
    ...
    for i, ln in enumerate(lines):
        end = lines[i + 1]["time"] if i + 1 < len(lines) else song_len
        lyric_items.append({"start": ln["time"], "end": min(end, song_len),
                            "text": ln["text"]})
    ...
    if url:
        res = ji.build_chart_chords(job, url, job_dir=job_dir)
        chords = res["chords"]
        job["chords"] = chords
