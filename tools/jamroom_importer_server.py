...
def _trigger_reaper_action(cfg, ext_key, receipt, timeout=60):
    """Fire one of our registered REAPER actions and wait for its receipt."""
    web = _reaper_web(cfg)

    def web_get(path, t=5):
        try:
            r = requests.get(f"{web}/_/{path}", timeout=t)
            return r.text if r.status_code == 200 else None
        except requests.RequestException:
            return None

    def stored():
        t = web_get(f"GET/EXTSTATE/ReaSetJR/{ext_key}")
        if t:
            parts = t.strip().split("\t")
            if len(parts) >= 4 and parts[3].strip():
        ...
        if reaper_exe.exists():
            ...
            for _ in range(15):
                ...
                if cmd:
                    break
    if not cmd:
        raise RuntimeError("That REAPER action is not registered. Run "
                           "tools/jamroom_register_apply.lua once from REAPER's "
                           "Action list, then try again.")
    web_get(cmd, t=10)
    for _ in range(timeout):
        if receipt.exists():
            return receipt.read_text(encoding="utf-8").strip()
        time.sleep(1)
    return None
...
def rechord_song(name, chart_url, snap=True, force=False, key_offset=None):
    ...
    audio) and the chart for the chord NAMES, so a song imported with poor
    ...
    if not job.get("chords") and not job.get("chart_source") and not job.get("chords_detected"):
        raise RuntimeError(
            ...
            "re-label. Re-import it from the Fadr library (free) to get them.")

    # Where does this song sit in the project right now?
    ...
    job["chart"]["key_override"] = key_offset
    job["generation"] = {"generator": ji.chart_model.GENERATOR,
...
def project_identity(cfg=None):
    cfg = cfg or ji.load_config(None)
    reply = requests.get(f"{_reaper_web(cfg)}/_/GET/PROJEXTSTATE/ReaSet/projectId", timeout=5)
    reply.raise_for_status()
    fields = reply.text.strip().split("\t")
    if len(fields) < 4 or not re.fullmatch(r"[A-Za-z0-9_-]+", fields[3]):
        raise RuntimeError("Open ReaSet once and save the project to establish its identity.")
    return fields[3]
...
def _push_song_items(cfg, name, song, chords=None, lyric_lines=None,
    ...
    request = {"region": name, "id": song["id"], "start": song["start"],
               "end": song["end"], "project": song.get("project") or project_identity(cfg),
    ...
    if chords is not None:
        request["chords"] = [{"s": c["start"], "e": c["end"], "name": c["chord"]} for c in chords]
    ...
    def lua(v):
        ...
        return str(v)
    (TOOLDIR / "jamroom_pending_rechord.lua").write_text("return " + lua(request), encoding="utf-8")
    receipt.unlink(missing_ok=True)
    result = _trigger_reaper_action(cfg, "rechord_cmd", receipt)
    if not result:
        raise RuntimeError("REAPER has not confirmed the update. Retry resumes this operation.")
    response = json.loads(result)
    if response.get("operation") != operation or response.get("status") != "ok":
        raise RuntimeError(response.get("message", "Mismatched REAPER receipt"))
...
def relyric_song(name, record_id=None, offset=None):
    ...
    old way out was to delete the song and re-import it — which does not even
    work, because the saved job is kept deliberately and its completed lyrics
    stage is skipped on the way back in.

    Chords are re-placed at the same time. They are positioned against the
    words now, so new lyrics without new chord timing would leave the two
    disagreeing.
    """
    cfg = ji.load_config(None)
    target_project = project_identity(cfg)
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _body(self):
        ...
        if origin and urlparse(origin).netloc != self.headers.get("Host"):
            raise ValueError("Request origin does not match this importer")
        n = int(self.headers.get("Content-Length") or 0)
        if n < 0 or n > 1048576:
            raise ValueError("Request body is too large")
        return json.loads(self.rfile.read(n) or b"{}")

    def _claim_maintenance(self):
        """Exclusively claim a project mutation outside the import pipeline."""
        if STATE["state"] in ACTIVE_STATES:
            self._send(409, {"error": "Finish or cancel the current import "
                                      "before changing another song."})
            return False
    ...
    def do_POST(self):
        try:
            ...
            if self.path == "/api/updates/start":
            ...
            elif self.path == "/api/updates/protect":
                UPDATES.protect(int(body["id"]), str(body["name"]), bool(body["keep"]), body.get("project"))
                self._send(200, {"ok": True})
                ...
                try:
                    ...
                    self._send(500, {"error": str(e), "log": STATE["log"]})
                finally:
                    BUSY.release()
            elif self.path == "/api/relyric":
                if not self._claim_maintenance():
                    return
                try:
                    STATE["log"] = []
