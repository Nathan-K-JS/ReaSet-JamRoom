            ...
            for name, engine, args, device in engines:
                with self.subTest(browser=name):
                    ...
                    try:
                        ...
                        self.assertTrue(page.evaluate('document.documentElement.scrollWidth<=innerWidth'))
                        page.goto(base+'/recordings?project=project&job=copy&return='+base+'/ReaSet.html')
                        page.get_by_role('link',name='Back to recording',exact=True).wait_for()
                        self.assertEqual(page.locator('#return-link').get_attribute('href'),base+'/ReaSet.html')
                        self.assertEqual(page.locator('#jobs article').count(),1)
                        page.get_by_role('link',name='Show QR',exact=True).click()
                        page.wait_for_function('document.getElementById("sharing").open')
