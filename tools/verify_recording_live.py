...
local function run()
  ...
  elseif stage==27 and reaper.time_precise()-started>1 then
    ...
    local before=#s.takes;local wasSelected=core.db.inputs[5].selected
    core:command({project=core.id,revision=core.revision,op='prepareTake',kind='new',session=s.id,take=t.id})
    check(reaper.GetPlayState()==0 and #s.takes==before and t.status=='kept','New take preparation does not record or change saved takes')
    core:command({project=core.id,revision=core.revision,op='select',input='5',value=false})
    core:command({project=core.id,revision=core.revision,op='cancelPrepare'})
    check(core.take==t.id and core.db.inputs[5].selected==wasSelected and not core.preparation,'Cancel preparation restores review and input choices')
    core:command({project=core.id,revision=core.revision,op='prepareTake',kind='redo',session=s.id,take=t.id})
    check(reaper.GetPlayState()==0 and t.status=='kept' and #s.takes==before,'Redo preparation retains the old take until recording starts')
    core:command({project=core.id,revision=core.revision,op='cancelPrepare'})
    core:command({project=core.id,revision=core.revision,op='prepareTake',kind='part',session=s.id,take=t.id})
    ...
    core:command({project=core.id,revision=core.revision,op='recordPrepared',stopAtEnd=false})
    local _,prepared=M.mix_take(core)
    check(prepared.backingOn==core.backingOn,'Prepared backing choice reaches the captured take')
